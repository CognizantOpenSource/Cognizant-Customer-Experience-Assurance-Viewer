...
public class GitLabAPIController {
    ...
    public Map<String, List> getProjectNames() {
        ...
        SCMCredDetails scm = commonService.getSCMCredDetails(SCMConstants.GITLAB);
        return gitLabService.getProjectDetails(scm.getApiOrHostUrl(), scm.getUsername(), scm.getToken());
    }
    ...
    public ResponseEntity testConnection(@RequestBody SCMCredDetails scmCredDetails) {
        return gitLabService.testConnection(scmCredDetails.getApiOrHostUrl(), scmCredDetails.getUsername(), scmCredDetails.getToken());
    }
